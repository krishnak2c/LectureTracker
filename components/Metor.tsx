  ...
  return (
    <div className="border rounded-3xl h-96 w-full relative flex justify-center items-center text-center bg-primary-foreground overflow-hidden">
      {meteorsData.map((meteor, index) => (
        <span
          key={index}
          className="absolute top-1/2 left-1/2 h-0.5 w-0.5 rotate-[10deg] animate-meteor rounded-[9999px] bg-sky-500 shadow-[0_0_0_1px_#ffffff10]"
          style={{
            top: "-5px",
            left: `${meteor.left}px`,
      ...
      <div className="space-y-6 z-10">
        <p>
          <Balancer className="justify-center flex text-3xl font-bold tracking-tight sm:text-4xl text-white z-10">
            It&apos;s Open Source!
          </Balancer>
        </p>
        <Button
          className="rounded-full hover:ring-4 bg-slate-800 hover:bg-sky-600"
          variant="secondary"
        >
          <a
            className="flex max-w-fit items-center justify-center space-x-2 rounded-full px-5 py-2 text-sm"
            href="https://github.com/krishnak2c/LectureTracker"
            target="_blank"
